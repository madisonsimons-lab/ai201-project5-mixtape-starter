...
I used Claude Code throughout — for orientation, for tracing call chains, and for forming initial hypotheses about each bug — but every hypothesis it produced from reading code was checked by actually running something (the existing test suite, a manual REPL-style repro script, or a live `curl` request against the running dev server) before I trusted it or fixed anything.

**Where it worked well — orientation and tracing.** I had it read `README.md` first, then all of `app.py`, `models.py`, every file in `routes/`, and every file in `services/` together, and asked it to explain what each module was responsible for and how a request flows end to end (e.g. "trace how rating a song reaches a service function, and contrast that with adding a song to a playlist"). That side-by-side trace is what first surfaced issue #4: `add_to_playlist()` calls `create_notification()` and `rate_song()` doesn't, even though they're structurally the same kind of event. Reading two similar functions side by side and asking what's structurally different between them was the single most useful move of the whole project.

**Where it helped explain, not diagnose.** For the streak bug, once the suspicious line (`elif days_since_last == 1 and today.weekday() != 6`) was already found by reading the function against its own docstring, I asked it to confirm what `datetime.weekday()` returns for Sunday (6) versus other conventions like `isoweekday()` (7) — a quick fact check, not a diagnosis. I verified the actual behavior myself afterward by running the existing `test_streak_increments_on_sunday` test and, separately, by driving `update_listening_streak()` with real Saturday/Sunday timestamps against a real seeded user record.

**Where it was wrong, and I caught it by running the code.** For issue #3 (duplicate songs in search), my first-pass read of `search_songs()` — an `outerjoin` against `song_tags` with no `.distinct()` — looked exactly like the reported bug: joining a song against its tags without deduplicating should return one row per tag, i.e. duplicates for any multi-tag song. That diagnosis was plausible and wrong. Running `pytest tests/test_search.py` showed all 5 tests passing, including the one explicitly commented `# Should be 1, bug causes it to be 3`. I didn't accept that at face value either — I wrote a small script that ran the *exact* compiled SQL from `search_songs()` directly against the database and compared the raw row count (3, as expected for a 3-tag song) to the ORM's `.all()` result (1). That confirmed SQLAlchemy's legacy `Query` API auto-deduplicates full-entity results by primary key, which the join-without-distinct pattern relies on by accident rather than by design. So the code is fragile, but it isn't the currently-observable bug — I only found this out by actually executing the query two different ways, not by re-reading the code more carefully. I left issue #3 unfixed and flagged it as needing further investigation rather than shipping a fix for a bug I couldn't actually trigger.

**Where I had to infer past a documentation gap.** No formal issue-description "brief" exists as a file in this repo — only the one-line summaries in `README.md`'s issue table. For issue #2 (feed showing "yesterday"), I couldn't cross-check my hypothesis against a fuller description, so I leaned on `seed_data.py`'s own comments (`# Older events ... should NOT appear in "listening now" after fix`) as an implicit spec and reproduced the behavior manually rather than via a written test (none exists for `feed_service.py`). I'm treating that diagnosis as "likely, not certain" in the writeup for exactly this reason, and it's why I deprioritized it behind the three bugs I could verify with higher confidence.
